...
import { ReceivingDataTableToolbar } from "./receiving-data-table-toolbar";
import { DataTablePagination } from "./data-table-pagination";
import type { Receiving, ReceivingFilters } from "@/lib/types/receiving";

interface ReceivingDataTableProps<TData, TValue> {
  ...
  onAdd: () => void;
  onServerSearch?: (filters: ReceivingFilters) => void;
  onServerSearchClear?: () => void;
}

export function ReceivingDataTable<TData, TValue>({
  columns,
  data,
  onView,
  onEdit,
  ...
  onServerSearch,
  onServerSearchClear,
}: ReceivingDataTableProps<TData, TValue>) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="lg:hidden space-y-3">
        {table.getRowModel().rows?.length ? (
          table.getRowModel().rows.map((row) => {
            const receiving = row.original as Receiving;
            return (
              <div
                key={row.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-[#1a1f36] font-mono">{receiving.code}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-[#64748b]">{receiving.supplier_name || "No supplier"}</span>
                      </div>
                    </div>
                  </div>
