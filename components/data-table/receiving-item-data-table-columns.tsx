...
export function createReceivingItemColumns(
  onEdit: (item: ReceivingItem) => void,
  onDelete: (item: ReceivingItem) => void
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const original = row.original as ReceivingItem;
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm tabular-nums text-[#1a1f36] text-right block">
          {(row.getValue("unit_price") as number).toFixed(2)}
        </span>
      ),
    },
    {
      ...
      header: "Total Cost",
      cell: ({ row }) => (
        <span className="text-sm tabular-nums text-[#1a1f36] text-right block font-medium">
          {(row.getValue("total_cost") as number).toFixed(2)}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="text-[#64748b]">
            {original.storage_location_name || "N/A"}
          </span>
        );
      },
    },
