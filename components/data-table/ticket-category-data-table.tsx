...
import { TicketCategoryDataTableToolbar } from "./ticket-category-data-table-toolbar";
import { DataTablePagination } from "./data-table-pagination";
import type { TicketCategory, TicketCategoryAdvancedFilter, TicketCategoryFilters } from "@/lib/types/ticket-category";

interface TicketCategoryDataTableProps<TData, TValue> {
...
export function TicketCategoryDataTable<TData, TValue>({
  columns,
  data,
  onView,
  onEdit,
  ...
  onServerSearch,
  onServerSearchClear,
}: TicketCategoryDataTableProps<TData, TValue>) {
  ...
  const [advancedFilters, setAdvancedFilters] = useState<TicketCategoryAdvancedFilter[]>([]);
  ...
  return (
    <div className="space-y-4">
      <TicketCategoryDataTableToolbar
        table={table}
        onView={onView}
        onEdit={onEdit}
        onDelete={onDelete}
      ...
      <div className="lg:hidden space-y-3">
        {table.getRowModel().rows?.length ? (
          table.getRowModel().rows.map((row) => {
            const ticketCategory = row.original as TicketCategory;
            return (
              <div
                key={row.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-[#1a1f36]">{ticketCategory.name}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => onView(ticketCategory)}
                      className="p-2 hover:bg-[#f0f4f8] transition-colors"
                    >
                      <span className="text-[#64748b]">👁️</span>
                    </button>
                    <button
                      onClick={() => onEdit(ticketCategory)}
                      className="p-2 hover:bg-[#f0f4f8] transition-colors"
                    >
                      <span className="text-[#64748b]">✏️</span>
                    </button>
                    {!ticketCategory.deleted_at ? (
                      <button
                      ...
                      <button
                        onClick={() => onRestore(ticketCategory)}
