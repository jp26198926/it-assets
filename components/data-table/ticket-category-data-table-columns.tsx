...
function Actions({ ticketCategory, onView, onEdit, onDelete, onRestore }: ActionsProps) {
  const { hasPermission } = useAuthorization();
  const canEdit = hasPermission("/ticket-categories", "Edit");
  const canDelete = hasPermission("/ticket-categories", "Delete");
  const canRestore = hasPermission("/ticket-categories", "Restore");
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-40 border-0 shadow-lg">
        <DropdownMenuItem onClick={() => onView(ticketCategory)} className="cursor-pointer gap-2 text-[#1a1f36]">
          <Eye className="h-4 w-4 text-[#64748b]" />
          View Details
        </DropdownMenuItem>
        {canEdit && (
          <DropdownMenuItem onClick={() => onEdit(ticketCategory)} className="cursor-pointer gap-2 text-[#1a1f36]">
...
export function createTicketCategoryColumns(
  onView: (ticketCategory: TicketCategory) => void,
  onEdit: (ticketCategory: TicketCategory) => void,
  onDelete: (ticketCategory: TicketCategory) => void,
  onRestore: (ticketCategory: TicketCategory) => void
): ColumnDef<TicketCategory>[] {
  return [
    {
      accessorKey: "name",
      header: ({ column }) => (
