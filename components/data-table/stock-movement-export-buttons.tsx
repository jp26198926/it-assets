...
interface StockMovementExportButtonsProps {
  table: Table<StockMovement>;
}

export function StockMovementExportButtons({ table }: StockMovementExportButtonsProps) {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("Stock Movements Report", 14, 22);
      doc.setFontSize(10);
      ...
      const rows = table.getFilteredRowModel().rows.map((row) => [
        ...
        row.original.reference_description || "N/A",
        row.original.remarks || "N/A",
      ]);

      autoTable(doc, {
        startY: 35,
        head: [["Date", "Type", "Item Code", "Item Name", "Location", "Qty", "Reference", "Remarks"]],
        body: rows,
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const rows = table.getFilteredRowModel().rows.map((row) => ({
        ...
        Type: row.original.transaction_type,
        ...
        Reference: row.original.reference_description || "N/A",
