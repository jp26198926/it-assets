...
async function generateReleasingItemCode(): Promise<string> {
  const lastItem = await ReleasingItemModel.findOne({ code: { $regex: "^OUT\\d{5}$" } })
    .sort({ code: -1 })
    .lean();
  ...
  return `OUT${String(nextNumber).padStart(5, "0")}`;
}

function toReleasingItem(d: Record<string, unknown>): ReleasingItem {
  ...
  const locId = d.from_location_id as unknown as
  ...
  if (locId && typeof locId === "object" && "_id" in locId) {
    from_location_id = locId._id.toString();
    from_location_name = locId.name;
  ...
  return {
    ...
    releasing_id: (d.releasing_id as { toString(): string }).toString(),
    item_id,
    item_name,
    item_code,
    item_uom_name,
...
function applyPopulates(query: ReturnType<typeof ReleasingItemModel.find>) {
  return query
    .populate({
      path: "item_id",
...
export async function getReleasingItems(releasingId: string): Promise<ReleasingItem[]> {
  ...
  const items = await applyPopulates(
    ReleasingItemModel.find({ releasing_id: releasingId })
      .sort({ created_at: 1 })
      .lean()
  );

  return items.map((d) => toReleasingItem(d as unknown as Record<string, unknown>));
}

export async function getReleasingItemById(id: string): Promise<ReleasingItem | null> {
  await connectDB();

  const item = await applyPopulates(ReleasingItemModel.findById(id).lean());

  if (!item) return null;

  return toReleasingItem(item as unknown as Record<string, unknown>);
}

export async function createReleasingItem(data: CreateReleasingItemInput): Promise<ReleasingItem> {
  await connectDB();

  const code = await generateReleasingItemCode();

  // Get from_location_id from parent releasing
  const { Releasing: ReleasingModel } = await import("@/lib/db/models/releasing");
  const releasing = await ReleasingModel.findById(data.releasing_id).lean();
  const from_location_id = releasing ? (releasing as unknown as { from_location_id: { toString(): string } }).from_location_id?.toString() || null : null;

  const item = await ReleasingItemModel.create({
    code,
    releasing_id: data.releasing_id,
  ...
  const created = await applyPopulates(ReleasingItemModel.findById(item._id).lean());

  if (!created) throw new Error("Failed to create releasing item");

  return toReleasingItem(created as unknown as Record<string, unknown>);
}

export async function updateReleasingItem(id: string, data: UpdateReleasingItemInput): Promise<ReleasingItem> {
  await connectDB();

  const existing = await ReleasingItemModel.findById(id);
  if (!existing) throw new Error("Releasing item not found");
  if (existing.status !== "Active") throw new Error("Can only edit Active releasing items");
  ...
  const item = await applyPopulates(
    ReleasingItemModel.findByIdAndUpdate(id, updateData, { new: true }).lean()
  );

  if (!item) throw new Error("Releasing item not found");
...
export async function deleteReleasingItem(id: string, reason?: string): Promise<void> {
  ...
  const existing = await ReleasingItemModel.findById(id);
  if (!existing) throw new Error("Releasing item not found");
  if (existing.status !== "Active") throw new Error("Can only delete Active releasing items");

  await ReleasingItemModel.findByIdAndUpdate(id, {
    deleted_at: new Date(),
