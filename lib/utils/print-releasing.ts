...
export async function printReleasing(
  releasing: Releasing,
  items: ReleasingItem[]
): Promise<void> {
  ...
  const itemRows = items
    .map(
      ...
      <tr>
        ...
        <td style="padding: 8px; border: 1px solid #e5e7eb;">${item.from_location_name || "N/A"}</td>
  ...
  const html = `
    ...
    <html>
    <head>
      <title>Releasing ${releasing.code}</title>
      <style>
        @media print {
    ...
    <body>
      ...
      <div class="header">
        ...
        <div class="releasing-no">
          <div class="label">Releasing No.</div>
          <div class="code">${releasing.code}</div>
        </div>
      </div>
      ...
      <div class="info-grid">
        <div class="info-row">
          ...
          <span class="info-value">${formatInAppTimezone(releasing.date_released, "yyyy-MM-dd", timezone)}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Status:</span>
          <span class="info-value">
            <span class="status-badge" style="background: ${statusColors[releasing.status] || "#64748b"}">
              ${releasing.status.toUpperCase()}
            </span>
          </span>
        </div>
        <div class="info-row">
          <span class="info-label">From Location:</span>
          <span class="info-value">${releasing.from_location_name || "N/A"}</span>
        </div>
        <div class="info-row">
          <span class="info-label">To Department:</span>
          <span class="info-value">${releasing.to_department_name || "N/A"}</span>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>
          ${itemRows || '<tr><td colspan="8" style="padding: 20px; text-align: center; color: #94a3b8;">No items</td></tr>'}
        </tbody>
      </table>

      <div class="remarks-box">
        <div class="remarks-label">REMARKS:</div>
        <div>${releasing.remarks || ""}</div>
      </div>
