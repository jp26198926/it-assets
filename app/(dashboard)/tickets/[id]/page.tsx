...
export default function TicketDetailPage() {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      try {
        ...
        setAppSettings({ app_name: settings.app_name, tagline: settings.tagline });
        if (data?.asset_id) {
  ...
  const handlePrint = () => {
    ...
    const conversationHtml = comments.length > 0 ? (() => {
      const logSectionNum = ticket.asset_name ? "Section 5" : "Section 4";
      const commentBlocks = comments.map((c) => {
        const text = (c.message.replace(/<[^>]*>/g, "") || "(no text)").replace(/</g, "&lt;").replace(/>/g, "&gt;");
        return `
    ...
    const descImages = ticket.description.replace(/<img[^>]*src="([^"]*)"[^>]*>/gi, '<div class="desc-img"><img src="$1" /></div>');
  ...
  return (
    <PageGuard pagePath="/tickets">
      ...
      <div className="print-page print-only">
        <div className="pf-header">
          <div className="pf-header-top">
            ...
            <div className="pf-header-center">
              ...
              <div className="pf-department">Information Technology Department</div>
            </div>
            <div className="pf-doc-meta">
              <div className="pf-doc-row"><span className="pf-doc-label">Doc Ref:</span> IT-TICKET-{ticket.ticket_no}</div>
              <div className="pf-doc-row"><span className="pf-doc-label">Rev:</span> 1.0</div>
              <div className="pf-doc-row"><span className="pf-doc-label">Page:</span> 1 of 1</div>
            </div>
          </div>
          <hr className="pf-hr-double" />
          <div className="pf-form-title">IT SUPPORT TICKET</div>
          <div className="pf-form-subtitle">Incident Report &amp; Resolution Form</div>
          <hr className="pf-hr-double" />
        </div>

        <div className="pf-section">
          <div className="pf-section-header">Section 1 — Ticket Information</div>
          <table className="pf-table">
            <tbody>
              <tr>
                ...
                <td className="pf-tr">{format(new Date(ticket.created_at), "dd-MMM-yyyy")}</td>
              </tr>
              <tr>
              ...
              {(ticket.status === "Resolved" || ticket.status === "Closed") && (
                <tr>
                  ...
                  <td className="pf-tr">{ticket.updated_at ? format(new Date(ticket.updated_at), "dd-MMM-yyyy hh:mm a") : "N/A"}</td>
          ...
          <table className="pf-table">
            <tbody>
              ...
              <tr>
                ...
                <td className="pf-tr" colSpan={3}>{ticket.assigned_to_name || "Unassigned"}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        {ticket.asset_name && (
          <div className="pf-section">
            <div className="pf-section-header">Section 3 — Asset Information</div>
            <table className="pf-table">
              <tbody>
                <tr>
        ...
        <div className="pf-section">
          <div className="pf-section-header">{ticket.asset_name ? "Section 4" : "Section 3"} — Issue Description</div>
        ...
        {ticket.attachments && ticket.attachments.length > 0 && (
          <div className="pf-section">
            <div className="pf-section-header">{ticket.asset_name ? "Section 5" : "Section 4"} — Attachments</div>
            <table className="pf-table pf-table-full">
              <thead>
                <tr>
                  <th className="pf-th" style={{ width: 30 }}>#</th>
        ...
        {comments.length > 0 && (
          <div className="pf-section">
            <div className="pf-section-header">{ticket.asset_name ? "Section 6" : "Section 5"} — Work Log</div>
            <table className="pf-table pf-table-full">
              <thead>
                <tr>
                  <th className="pf-th" style={{ width: 130 }}>Date &amp; Time</th>
                  <th className="pf-th" style={{ width: 120 }}>Personnel</th>
              ...
              <tbody>
                {comments.map((comment) => (
                  <tr key={comment.id}>
                    <td className="pf-td">{format(new Date(comment.created_at), "dd-MMM-yyyy hh:mm a")}</td>
                    <td className="pf-td">{comment.created_by_name || "Unknown"}</td>
                    <td className="pf-td" dangerouslySetInnerHTML={{ __html: comment.message }} />
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="pf-section pf-no-break">
          ...
          <div className="pf-signature-grid">
            ...
            <div className="pf-sig-block">
              ...
              <div className="pf-sig-name">{ticket.assigned_to_name || "N/A"}</div>
              <div className="pf-sig-date">Date: _______________</div>
            </div>
            <div className="pf-sig-block">
              <div className="pf-sig-line"></div>
        ...
        <div className="pf-footer">
          <div className="pf-footer-left">
            <span className="pf-footer-label">Printed:</span> {format(new Date(), "dd-MMM-yyyy h:mm a")}
          </div>
          <div className="pf-footer-center">CONFIDENTIAL — For Internal Use Only</div>
          ...
          <div className="space-y-6">
            ...
            {comments.length > 0 && (
              <div className="space-y-3">
                {comments.map((comment) => (
                  <div key={comment.id} className="bg-white shadow-sm rounded-xl p-6">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="size-8 rounded-full bg-[#3b82f6] flex items-center justify-center text-white text-xs font-medium">
                        {comment.created_by_name
                          ? comment.created_by_name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)
                          : "?"}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-[#1a1f36]">{comment.created_by_name || "Unknown"}</p>
              ...
              <div className="bg-white shadow-sm rounded-xl p-6">
                ...
                {attachments.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {attachments.map((file, index) => (
                      <div key={index} className="flex items-center justify-between gap-2 text-sm bg-[#f8fafc] rounded-md px-3 py-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="truncate text-[#1a1f36]">{file.name}</span>
                          <span className="text-xs text-[#64748b] shrink-0">({formatFileSize(file.size)})</span>
                        </div>
                        <button
                          type="button"
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="sm" className="gap-2" onClick={() => handleSubmitComment()} disabled={submitting || uploadingFiles || (!replyContent.trim() && attachments.length === 0)}>
                      <Send className="h-4 w-4" />
                      {submitting ? (uploadingFiles ? "Uploading..." : "Submitting...") : "Submit"}
                    </Button>
                    <Button size="sm" variant="outline" className="gap-2" onClick={() => handleSubmitComment(true)} disabled={submitting || uploadingFiles || (!replyContent.trim() && attachments.length === 0)}>
                      <Send className="h-4 w-4" />
                      Submit as Resolved
                    </Button>
                  </div>
          ...
          <div className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            ...
            <div className="bg-white shadow-sm rounded-xl p-4">
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" className="gap-2 flex-1" onClick={handlePrint}>
                ...
                <CollapsibleContent>
                  <div className="px-4 pb-4 space-y-3">
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <span className="text-sm text-[#1a1f36]">{commentTotal}</span>
                    </div>
                    <div className="flex items-start justify-between">
                      <span className="text-xs text-[#64748b]">Last replier</span>
                      <span className="text-sm text-[#1a1f36]">
                        {comments.length > 0 ? (comments[comments.length - 1].created_by_name || "Unknown") : "N/A"}
                      </span>
                    </div>
                  </div>
                </CollapsibleContent>
      ...
      <style jsx global>{`
        ...
        @media print {
          ...
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          .pf-header-top { display: flex; align-items: center; gap: 12px; }
          .pf-logo-box {
            width: 60px; height: 60px; border: 1px solid #000;
            display: flex; align-items: center; justify-content: center;
            font-size: 9px; font-weight: 700; color: #666; flex-shrink: 0;
          }
          .pf-header-center { flex: 1; }
          .pf-company-name { font-size: 15px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; }
          .pf-department { font-size: 10px; color: #555; margin-top: 1px; }
          .pf-doc-meta { text-align: right; font-size: 9px; line-height: 1.5; flex-shrink: 0; }
          .pf-doc-label { font-weight: 700; }
          .pf-hr-double { border: none; border-top: 3px double #000; margin: 6px 0; }
          .pf-form-title { text-align: center; font-size: 14px; font-weight: 700; text-transform: uppercase; letter-spacing: 3px; margin: 4px 0 2px; }
          .pf-form-subtitle { text-align: center; font-size: 9px; text-transform: uppercase; letter-spacing: 1px; color: #555; margin: 0 0 4px; }

          .pf-section { border: 1px solid #000; margin-bottom: 8px; page-break-inside: avoid; }
          .pf-section-header {
            background: #e8e8e8; font-size: 10px; font-weight: 700;
            text-transform: uppercase; letter-spacing: 0.8px;
            padding: 4px 8px; border-bottom: 1px solid #000;
          }
          .pf-table { width: 100%; border-collapse: collapse; }
          .pf-table-full { width: 100%; border-collapse: collapse; }
          .pf-tl {
            width: 22%; font-weight: 700; font-size: 9px; text-transform: uppercase;
            padding: 5px 8px; background: #f5f5f5; border: 1px solid #ccc;
            letter-spacing: 0.3px;
          }
          .pf-tr { width: 28%; font-size: 10px; padding: 5px 8px; border: 1px solid #ccc; }
          .pf-th {
            font-size: 9px; font-weight: 700; text-transform: uppercase;
            padding: 4px 8px; background: #e8e8e8; border: 1px solid #ccc;
            text-align: left; letter-spacing: 0.3px;
          }
          .pf-td { font-size: 10px; padding: 4px 8px; border: 1px solid #ccc; vertical-align: top; }
          .pf-td-center { font-size: 10px; padding: 4px 8px; border: 1px solid #ccc; text-align: center; }

          .pf-field-box { border: 1px solid #ccc; margin: 0; }
          .pf-field-box + .pf-field-box { border-top: none; }
          .pf-field-label {
            font-size: 9px; font-weight: 700; text-transform: uppercase;
            padding: 3px 8px; background: #f5f5f5; border-bottom: 1px solid #ccc;
            letter-spacing: 0.3px;
          }
          .pf-field-content { padding: 5px 8px; font-size: 10px; line-height: 1.5; min-height: 14px; }
          .pf-bold { font-weight: 600; }

          .pf-signature-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0; }
          .pf-sig-block {
            padding: 12px 16px; border: 1px solid #ccc; text-align: center;
          }
          .pf-sig-line { border-bottom: 1px solid #000; height: 32px; margin-bottom: 4px; }
          .pf-sig-role { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
          .pf-sig-name { font-size: 9px; color: #555; margin-top: 2px; }
          .pf-sig-date { font-size: 9px; color: #555; margin-top: 2px; }

          .pf-footer {
            display: flex; justify-content: space-between; align-items: center;
            font-size: 8px; color: #555; padding-top: 6px; margin-top: 4px;
          }
          .pf-footer-label { font-weight: 700; }
          .pf-footer-center { font-style: italic; text-transform: uppercase; letter-spacing: 0.5px; }
