...
import type { Receiving, CreateReceivingInput, ReceivingFilters } from "@/lib/types/receiving";
...
export default function ReceivingsPage() {
  ...
  const [cancelReceivingItem, setCancelReceivingItem] = useState<Receiving | null>(null);
  ...
  const handleServerSearch = useCallback((filters: ReceivingFilters) => {
    ...
    getReceivings(filters).then((data) => setReceivings(data)).catch(() => {
  ...
  const handleServerSearchClear = useCallback(() => {
    setActiveFilters({});
    getReceivings().then((data) => setReceivings(data)).catch(() => {
  ...
  const handleCancelConfirm = async (reason: string) => {
    if (cancelReceivingItem) {
      try {
        ...
        toast.success(`Receiving ${cancelReceivingItem.code} has been cancelled`);
  ...
  const columns = createReceivingColumns(handleView, handleEdit, handleCancel, appTimezone);
  ...
  if (loading) {
    return (
      <div className="space-y-4 sm:space-y-6">
        <ScrollReveal>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[#1a1f36] sm:text-3xl">Receivings</h1>
