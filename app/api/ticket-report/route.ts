...
export async function GET(request: NextRequest) {
  try {
    ...
    const [tickets, summary, totals] = await Promise.all([
      ticketReportService.getFilteredTickets(filters),
      ticketReportService.getTicketSummary(filters),
      ticketReportService.getTicketTotals(filters),
    ]);

    return apiSuccess({ tickets, summary, totals });
  } catch (error) {
    return apiError(error instanceof Error ? error.message : "Failed to fetch ticket report");
  }
}
